...
def load_model_single(model_name, num_classes, from_path=None):
    ...
    return model, hyperparameters, num_epochs, n_splits
...
# Iterate over each Dataset in the ConcatDataset
    ...
    for fold, (train_ids, val_ids) in enumerate(kfold.split(train_dataset, labels)):
        ...
        for epoch in range(num_epochs):
            ...
            print(f'Validation Loss: {val_epoch_loss:.4f}  |  ROC AUC: {roc_auc:.4f}')

            wandb.log({"train_loss": train_epoch_loss, "val_loss": val_epoch_loss, "roc_auc":roc_auc})
